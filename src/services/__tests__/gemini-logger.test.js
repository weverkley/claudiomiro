...
describe('Gemini Logger', () => {
  describe('Helper Functions - formatToolName', () => {
    test('should return Bash with tool icon', () => {
      ...
      const result = processGeminiMessage(JSON.stringify(json));
  ...
  describe('processAssistantMessage', () => {
    test('should process text message', () => {
      const json = {
        ...
        message: {
          content: [
            { type: 'text', text: 'Hello, I am Gemini' }
          ]
        }
      };
      const result = processGeminiMessage(JSON.stringify(json));
  ...
  describe('processGeminiMessage - Main Entry Point', () => {
    ...
    test('should handle malformed JSON', () => {
      const result = processGeminiMessage('{ "type": "assistant", "message": }');
      expect(result).toBeNull();
    });
    ...
    test('should handle null input', () => {
      const result = processGeminiMessage('null');
      expect(result).toBeNull();
    });
    ...
    test('should handle string input', () => {
      const result = processGeminiMessage('"just a string"');
      expect(result).toBeNull();
    });

    test('should handle number input', () => {
      const result = processGeminiMessage('123');
      expect(result).toBeNull();
    });
  });
